...
import { createCompactionAutocontinueHandler, createSessionCompactingHandler, type CompactionAutocontinueHook } from "../plugin/session-compacting";
...
import { maybeAutoInitSecondBrain } from "../tools/second-brain";
...
export function createPluginModule(overrides: Partial<PluginModuleDeps> = {}): PluginModule {
  ...
  const serverPlugin: Plugin = async (input, _options): Promise<Hooks> => {
    ...
    maybeAutoInitSecondBrain(input.directory, pluginConfig.second_brain);
    ...
    if (pluginConfig.team_mode?.enabled) {
      ...
      try {
        ...
        if (pluginConfig.disabled_skills?.includes("team-mode")) {
          console.warn("[team-mode] enabled=true but team-mode skill is disabled; skill docs hidden but tools still registered (D-29)");
        }
      } catch (error) {
