  ...
  return async (input, output): Promise<void> => {
    ...
    if (input.tool.toLowerCase() === "bash" && typeof output.args.command === "string") {
      ...
      if (
        ...
        (backgroundManager?.hasActiveChildTasks(input.sessionID) === true || backgroundManager?.hasPendingParentWake(input.sessionID) === true)
      ) {
    ...
    if (input.tool === "task") {
      ...
      if (shouldInjectOracleVerification) {
        ...
        replaceToolArgs(output, {
          ...
          prompt: buildUltraworkOracleVerificationPrompt(prompt, loopState.prompt, verificationAttemptId),
        });
      }
    }
    ...
    if (hooks.ralphLoop && input.tool === "skill") {
      ...
      if (command === "ralph-loop" && sessionID) {
        ...
        const resumed = isRalphLoopResumeArgument(rawArgs) && hooks.ralphLoop.resumeLoop?.(sessionID) === true;
        if (!resumed) {
