...
function isRuntimeFallbackEnabled(hooks: ChatMessageHooks, pluginConfig: OhMyOpenCodeConfig): boolean {
  return (
    hooks.runtimeFallback !== null &&
    hooks.runtimeFallback !== undefined &&
    (typeof pluginConfig.runtime_fallback === "boolean" ? pluginConfig.runtime_fallback : (pluginConfig.runtime_fallback?.enabled ?? false))
  );
}
...
async function runChatMessageHooks(args: {
  ...
  await hooks.secondBrainInjector?.["chat.message"]?.(input, output);
...
export function createChatMessageHandler(args: {
  ...
  return async (input: ChatMessageInput, output: ChatMessageHandlerOutput): Promise<void> => {
