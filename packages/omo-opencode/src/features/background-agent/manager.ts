...
import { clearDelegatedChildSessionBootstrap, registerDelegatedChildSessionBootstrap } from "../../shared/delegated-child-session-bootstrap";
...
import { type BackgroundTaskNotificationTask, buildBackgroundTaskNotificationText } from "./background-task-notification-template";
...
import { type CircuitBreakerSettings, detectRepetitiveToolUse, recordToolCall, resolveCircuitBreakerSettings } from "./loop-detector";
...
import { createSubagentDepthLimitError, getMaxSubagentDepth, resolveSubagentSpawnContext, type SubagentSpawnContext } from "./subagent-spawn-limits";
...
import { archiveBackgroundTask, forgetBackgroundTask, getRegisteredBackgroundTask, rememberBackgroundTask } from "./task-registry";
...
const TERMINAL_BACKGROUND_TASK_STATUSES = new Set<BackgroundTask["status"]>(["completed", "error", "cancelled", "interrupt"]);
...
The fallback retry session is now created and can be inspected directly.
  ...
  private getRawConcurrencyKeyFromTask(task: Pick<BackgroundTask, "model" | "agent">): string {
    return task.model ? `${task.model.providerID}/${task.model.modelID}` : task.agent;
  }
  ...
  private parentWakeTextDeltaBufferKey(sessionID: string, partInfo: ReturnType<typeof resolveMessagePartInfo>): string {
  ...
  handleEvent(event: Event): void {
    ...
    if (event.type === "message.part.updated" || event.type === "message.part.delta") {
      ...
      const dispatchedWake = this.parentWakeNotifier.getDispatchedParentWakes().get(sessionID);
      const holdDispatchedWakeForTextDelta = this.shouldHoldDispatchedParentWakeForTextDelta(event.type, partInfo, sessionID, dispatchedWake);
      const hasParentWakeOutput =
        hasOutputSignalFromPart(partInfo, sessionID) && !isUserPart && !isInternalWakePart && !holdDispatchedWakeForTextDelta;
      ...
      if (partInfo?.type === "tool" || partInfo?.tool) {
        ...
        if (partInfo.tool) {
          ...
          task.progress.toolCallWindow = recordToolCall(task.progress.toolCallWindow, partInfo.tool, circuitBreaker, toolInput);
  ...
  private async interruptTaskFromAsyncPromptFailure(task: BackgroundTask, errorMessage: string, reason: string): Promise<void> {
...
The task was re-queued on a fallback model after a retryable failure.
    ...
    if (retried && retryingNotification) {
      const parentPromptContext = await this.resolveParentWakePromptContext(task);
      this.queuePendingParentWake(task.parentSessionId, retryingNotification, parentPromptContext, false, PENDING_PARENT_WAKE_DEBOUNCE_MS);
    }
  ...
  injectPendingNotificationsIntoChatMessage(
    _output: { parts: Array<{ type: string; text?: string; [key: string]: unknown }> },
    sessionID: string,
  ): void {
  ...
  private async validateSessionHasOutput(sessionID: string): Promise<boolean> {
    ...
    try {
      ...
      const hasAssistantOrToolMessage = messages.some((m: { info?: { role?: string } }) => m.info?.role === "assistant" || m.info?.role === "tool");
      ...
      const hasContent = messages.some((m: SessionMessage) => {
        ...
        return parts.some(
          (p: SessionPart) =>
            ...
            (p.type === "tool_result" && p.content && (typeof p.content === "string" ? p.content.trim().length > 0 : p.content.length > 0)),
        );
      });
  ...
  private async notifyParentSession(task: BackgroundTask): Promise<void> {
    ...
    if (pendingSet) {
      ...
      remainingCount = Array.from(this.tasks.values()).filter(
        (t) => t.parentSessionId === task.parentSessionId && t.id !== task.id && (t.status === "running" || t.status === "pending"),
      ).length;
      allComplete = remainingCount === 0;
    }
    ...
    const completedTasks = allComplete
      ? (this.completedTaskSummaries.get(task.parentSessionId) ?? [
          { id: task.id, description: task.description, status: task.status, error: task.error, attempts: cloneAttempts(task) },
        ])
      : [];
    ...
    if (this.enableParentSessionNotifications) {
      ...
      if (shouldDeferNotification) {
        this.queuePendingParentWake(task.parentSessionId, notification, parentPromptContext, shouldReply, PENDING_PARENT_WAKE_DEBOUNCE_MS);
      ...
      for (const task of this.tasks.values()) {
        ...
        try {
          ...
          if (sessionStatus?.type === "retry") {
            const retryMessage =
              typeof (sessionStatus as { message?: string }).message === "string" ? (sessionStatus as { message?: string }).message : undefined;
