...
const getModelCapabilitiesMock = mock(() => ({ maxOutputTokens: undefined as number | undefined }));

mock.module("../shared/model-capabilities", () => ({
...
describe("preemptive-compaction", () => {
  ...
  beforeEach(() => {
    ...
    getModelCapabilitiesMock.mockReturnValue({ maxOutputTokens: undefined });
    delete process.env[ANTHROPIC_CONTEXT_ENV_KEY];
    delete process.env[VERTEX_CONTEXT_ENV_KEY];
  });
  ...
  // #given input alone is below the 78% threshold but input + reserved completion budget exceeds it
  // #when tool.execute.after runs for a custom model with a large max output reservation
  // #then compaction should trigger early so the request never overflows the context window
  it("should reserve the model's output budget when deciding to compact", async () => {
    getModelCapabilitiesMock.mockReturnValue({ maxOutputTokens: 32000 });
    const modelCacheState = {
      anthropicContext1MEnabled: false,
      modelContextLimitsCache: new Map<string, number>([["custom/qwen3.6-27b-fp8", 131072]]),
    };
    const hook = createPreemptiveCompactionHook(ctx as never, {} as never, modelCacheState);
    const sessionID = "ses_reserved_output";

    // 72000 input = 55% of 131072 (below 78% on its own), but 72000 + 32000 = 79.3%
    await hook.event({
      event: {
        type: "message.updated",
        properties: {
    ...
    await hook["tool.execute.after"]({ tool: "bash", sessionID, callID: "call_1" }, { title: "", output: "test", metadata: null });

    expect(ctx.client.session.summarize).toHaveBeenCalled();
  });

  // #given the same input but no output reservation known for the model
  // #when tool.execute.after runs
  // #then input alone stays below the 78% threshold and compaction does not trigger
  it("should not compact on input alone when it stays below the threshold", async () => {
    getModelCapabilitiesMock.mockReturnValue({ maxOutputTokens: undefined });
    const modelCacheState = {
      anthropicContext1MEnabled: false,
      modelContextLimitsCache: new Map<string, number>([["custom/qwen3.6-27b-fp8", 131072]]),
    ...
    const hook = createPreemptiveCompactionHook(ctx as never, {} as never, modelCacheState);
    const sessionID = "ses_no_reservation";
  ...
  it("should log summarize errors instead of swallowing them", async () => {
    ...
    //#when
    await hook["tool.execute.after"]({ tool: "bash", sessionID, callID: "call_log" }, { title: "", output: "test", metadata: null });
  ...
  it("should show a warning toast when preemptive compaction fails", async () => {
    ...
    //#when
    await hook["tool.execute.after"]({ tool: "bash", sessionID, callID: "call_toast" }, { title: "", output: "test", metadata: null });
  ...
  it("should enforce cooldown even after failed compaction to prevent rapid retry loops", async () => {
    ...
    await hook["tool.execute.after"]({ tool: "bash", sessionID, callID: "call_fail" }, { title: "", output: "test", metadata: null });
    ...
    await hook["tool.execute.after"]({ tool: "bash", sessionID, callID: "call_fail_2" }, { title: "", output: "test", metadata: null });
  ...
  it("should clear in-progress lock when summarize times out", async () => {
    ...
    ctx.client.session.summarize.mockImplementationOnce(() => new Promise(() => {})).mockResolvedValueOnce({});
    ...
    try {
      ...
      //#when
      await hook["tool.execute.after"]({ tool: "bash", sessionID, callID: "call_timeout_1" }, { title: "", output: "test", metadata: null });
      ...
      try {
        ...
        await hook["tool.execute.after"]({ tool: "bash", sessionID, callID: "call_timeout_2" }, { title: "", output: "test", metadata: null });
  ...
  it("should allow re-compaction when context grows after successful compaction", async () => {
    ...
    await hook["tool.execute.after"]({ tool: "bash", sessionID, callID: "call_2" }, { title: "", output: "test", metadata: null });
